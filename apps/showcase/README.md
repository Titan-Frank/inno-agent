...
- **渲染**：消息气泡直接 import 主应用（`inno-agent-web`）源码里的 `react/chat/MessageBubble` 等**纯 props 驱动组件**（经 Vite alias `@inno-web` 指向 `apps/inno-agent/web/src`)，样式表也整个复用。主应用前端更新后，重新 build 本站即同步。
- **回放**：按消息真实时间间隔（钳制在 350ms–2.5s）逐条 reveal，支持播放/暂停、1×/2×/4× 倍速、进度条拖拽、跳转结尾。
